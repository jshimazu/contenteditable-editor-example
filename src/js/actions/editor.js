...
function filterHtml(beforeHTML) {
    ...
    // embed
    afterHTML = filterEmbed(afterHTML);

    // url
    afterHTML = filterLink(afterHTML);

    return afterHTML;
};

function filterEmbed(beforeHTML) {
    let afterHTML = beforeHTML;
    // embed
...
function filterLink(beforeHTML) {
    let afterHTML = beforeHTML;

    let match;
    if (match = beforeHTML.match(/((http|https):\/\/.+\.[a-z]{2,3})/)) {
        console.log(match);
        beforeHTML.replace();
    }
    return afterHTML;
